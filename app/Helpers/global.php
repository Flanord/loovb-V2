...
function get_posttypes($option = false)
{
    $services = [];
...
function list_pays($key = null)
{
    $pays = [];
    $pays_data = file_get_contents(public_path('vendors/countries/pays.json'));
    $pays_data = json_decode($pays_data, true);
    if (!empty($pays_data) && is_array($pays_data)) {
        foreach ($pays_data as $countrys) {
            $pays[$countrys['id']] = [
                'name' => $countrys['name'],
                'flag128' => '<img class="mr-1" src="' . asset('vendors/countries/flag/128x128/' . $countrys['alpha2'] . '.png') . '">',
                'flag64' => '<img class="mr-1" src="' . asset('vendors/countries/flag/64x64/' . $countrys['alpha2'] . '.png') . '">',
                'flag48' => '<img class="mr-1" src="' . asset('vendors/countries/flag/48x48/' . $countrys['alpha2'] . '.png') . '">',
                'flag32' => '<img class="mr-1" src="' . asset('vendors/countries/flag/32x32/' . $countrys['alpha2'] . '.png') . '">',
                'flag24' => '<img class="mr-1" src="' . asset('vendors/countries/flag/24x24/' . $countrys['alpha2'] . '.png') . '">',
                'flag16' => '<img class="mr-1" src="' . asset('vendors/countries/flag/16x16/' . $countrys['alpha2'] . '.png') . '">',
            ];
        }
    }

    if ($key) {
        return (isset($pays_data[$key])) ? $countries[$key] : ['name' => '', 'flag' => ''];
    }
    return $countries;
}
