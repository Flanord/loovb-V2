...
class AuthController extends Controller
{
    ...
    public function _postSignUp(Request $request)
    {
        $validator = Validator::make($request->all(),
            ...
            [
                ...
                'password.min' => __('Le mot de passe comporte au moins 6 caractères'),
        ...
        if ($user) {
            return $this->sendJson([
                'status' => 0,
                'message' => view('common.alert', ['type' => 'danger', 'message' => __('Cet utilisateur existe déjà')])->render()
            ]);
        }
        try {
        ...
        if (!$user) {
            ...
            return $this->sendJson([
                'status' => 0,
                'message' => view('common.alert', ['type' => 'success', 'message' => __('Enregistré avec succès')])->render(),
    ...
    public function _postResetPassword(Request $request)
    {
        ...
        if (is_null($user)) {
            return $this->sendJson([
                'status' => 0,
                'message' => view('common.alert', ['type' => 'danger', 'message' => __('L\'email n\'existe pas')])->render()
            ]);
        } else {
            ...
            if (!$user) {
                return $this->sendJson([
                    'status' => 0,
                    'message' => view('common.alert', ['type' => 'danger', 'message' => __('Impossible de réinitialiser le mot de passe pour ce compte. Réessayer!')])->render()
                ]);
            } else {
                ...
                return $this->sendJson([
                    'status' => 0,
                    'message' => view('common.alert', ['type' => 'success', 'message' => __('Avec succès! Veuillez vérifier votre email pour un nouveau mot de passe.')])->render(),
