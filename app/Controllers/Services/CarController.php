...
class CarController extends Controller
{
	...
	public function _bulkActions(Request $request){
		...
		if(!empty($action) && !empty($post_id)){
			...
			$this->sendJson( [
				...
				'message' => __( 'Action groupée avec succès' )
			], true );
		}
		$this->sendJson( [
			'status'  => 0,
    ...
    public function _editCar(Request $request, $id = null)
    {
    	...
    	if(!is_null($carObject)) {
		    $folder = $this->getFolder();

		    return view( "dashboard.screens.{$folder}.services.car.edit-car", [ 'role'      => $folder,
