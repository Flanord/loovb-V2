...
<div class="single-page single-car pb-5">
    ...
    <div class="hh-gallery hh-thumbnail has-background-image" data-src="{{ $thumbnailUrl }}" style="background-image: url({{ $thumbnailUrl }})">
        <div class="controls">
            <a href="javascript: void(0);" class="view-gallery item-link"><span>{{__('Voir les photos')}}</span> <i class="ti-gallery"></i> </a>
            @if(!empty($post->video))
                @php
                    enqueue_script('magnific-popup-js');
                    enqueue_style('magnific-popup-css');
                @endphp
                <a href="{{$post->video}}" class="view-video item-link ml-1 hh-iframe-popup" data-effect="mfp-zoom-in"><span>{{__('Voir la vidéo')}}</span> <i class="ti-video-clapper"></i> </a>
            @endif
        </div>
        @php
            if ( !empty( $gallery ) ) {
    ...
    <div class="container">
            ...
            <div class="col-12 col-sm-8 col-md-8 col-lg-9 col-content">
                ...
                <h1 class="title mt-3">
                    ...
                    @if($post->is_featured == 'on')
                        <span class="is-featured">{{ get_option('featured_text', __('En vedette')) }}</span>
                    @endif
                </h1>
                ...
                @endif
                <h2 class="heading mt-3 mb-2">{{__('Stratégies')}}</h2>
                @php
                            ...
                            <div class="tab-content">
                                ...
                                @if($booking_form == 'instant_enquiry' || $booking_form == 'instant')
                                    <div
                                        ...
                                        <form class="form-action" action="{{ url('add-to-cart-car') }}" method="post" data-loading-from=".form-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="form-group form-group-date-time @if($booking_type == 'hour' && (empty($checkInTime) || empty($checkOutTime))) d-none @endif">
                                                ...
                                                <div class="date-wrapper date-time">
                                                        ...
                                                        <div class="dropdown-time">
                                                            @if($booking_type == 'day' || (!empty($checkInTime) && !empty($checkOutTime)))
                                                                @foreach($listTime as $key => $value)
                                                                    <div class="item @if($checkInTime == $key) active @endif" data-value="{{ $key }}">{{ $value }}</div>
                                                                @endforeach
                                                            @endif
                                                        </div>
                                                    ...
                                                    <div class="date-render check-out-render"
                                                        ...
                                                        <div class="dropdown-time">
                                                            @if($booking_type == 'day' || (!empty($checkInTime) && !empty($checkOutTime)))
                                                                @foreach($listTime as $key => $value)
                                                                    <div class="item @if($checkOutTime == $key) active @endif" data-value="{{ $key }}">{{ $value }}</div>
                                                                @endforeach
                                                            @endif
                                                        </div>
                                                        <input type="hidden" name="endTime" value="{{$checkOutTime}}" class="input-checkin"/>
                                                    </div>
                                                </div>
                                            </div>
                                                            ...
                                                            <div class="extra-item d-flex">
                                                                <div class="checkbox checkbox-success">
                                                                    ...
                                                                    <label
                                                                            ...
                                                                            <span class="name">{{ get_translate($val['title']) }}</span>
                                                                    </label>
                                                                </div>
                                                                <span
        ...
        @if(count($list_services['results']))
            <h2 class="heading mt-4 mb-2">{{__('Voitures à proximité')}}</h2>
    ...
    <div class="mobile-book-action">
        <div class="container">
            <div class="action-inner">
                ...
                <a class="btn btn-primary action-button" id="mobile-check-availability">{{__('Voir les disponibilités')}}</a>
            </div>
        </div>
    </div>
</div>
