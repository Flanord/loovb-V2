                ...
                <div class="card bg-pattern mt-5">
                    <div class="card-body p-4">
                        <div class="mt-3 text-center">
                            ...
                            <h3>{{ __('Votre compte a été créé avec succès!') }}</h3>
                            <?php
                                $userdata = get_user_by_id($user_id);
                            ?>
                            <p class="text-muted font-14 mt-2">{!! sprintf(__('Un courriel a été envoyé à <b>%s</b>.'), $userdata->email) !!}
                                <br/>{{__('Veuillez vérifier votre boîte de réception pour confirmer vos informations de connexion.')}}</p>
                            <a href="{{url('/')}}"
                               class="btn btn-block btn-pink waves-effect waves-light mt-3">{{__('Retour à la page d\'accueil')}}</a>
                        </div>
