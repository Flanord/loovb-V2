...
<div id="hh-options-wrapper" class="hh-options-wrapper">
    ...
    <div class="hh-options-tab" data-tabs-calculation>
        <div class="nav nav-pills nav-pills-tab @if ($addNew) disabled-link @endif" id="v-pills-tab" role="tablist" data-tabs
             aria-orientation="vertical">
            @foreach ($options['sections'] as $key => $section)
                @php $class = ($key == 0) ? 'active show' : ''; @endphp
                <a class="nav-link mb-2 {{ $class }}" data-tabs-item id="v-pills-{{ $section['id'] }}-tab" data-toggle="pill" href="#v-pills-{{ $section['id'] }}" role="tab" aria-controls="v-pills-{{ $section['id'] }}"
                   aria-selected="true"> @if (!empty($section['icon']))
        ...
        <div class="tab-content">
            ...
            @foreach ($options['sections'] as $key => $section)
                ...
                <div class="tab-pane fade {{ $class }}"
                    ...
                    <form class="form hh-options-form @if ($key < $totalTab - 1) save-and-next @endif form-translation"  action="{{ $action }}"  method="post" data-tab="#v-pills-{{ $section['id'] }}-tab">
                       <?php
                        ...
                        <input type="hidden" name="postEncrypt" value="{{ hh_encrypt($serviceObject->post_id) }}">
                        <input type="hidden" name="hh_options_available_fields" value="{{ base64_encode(serialize($availableField)) }}">
