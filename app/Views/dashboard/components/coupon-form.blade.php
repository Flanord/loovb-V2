...
<div class="form-group">
    ...
    <textarea name="coupon_description{{get_lang_suffix($item)}}" id="coupon_description{{get_lang_suffix($item)}}" class="form-control {{get_lang_class($key, $item)}}"
              placeholder="{{__('Enter more detail')}}" @if(!empty($item)) data-lang="{{$item}}" @endif>{{ get_translate($couponObject->coupon_description, $item) }}</textarea>
    @endforeach
</div>
<div class="row">
    <div class="col-12 col-sm-6">
        <div class="form-group">
            ...
            <select name="coupon_type" id="coupon_type_update" class="wide"
                ...
                <option value="fixed" @if($couponObject->price_type == 'fixed') selected @endif>{{ __('Fixed') }}</option>
                <option value="percent" @if($couponObject->price_type == 'percent') selected @endif>{{ __('Percent') }}</option>
            </select>
        </div>
    </div>
    <div class="col-12 col-sm-6">
