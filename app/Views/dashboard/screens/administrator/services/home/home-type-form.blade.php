...
<div class="form-group">
    ...
    <input type="text" class="form-control has-validation {{get_lang_class($key, $item)}}"
           data-validation="required" id="term_name_update{{get_lang_suffix($item)}}" name="term_name{{get_lang_suffix($item)}}"
           value="{{ get_translate($termObject->term_title, $item) }}" @if(!empty($item)) data-lang="{{$item}}" @endif
           placeholder="{{__('Nom')}}">
    @endforeach
...
<div class="form-group">
    ...
    @foreach($langs as $key => $item)
    <textarea name="term_description{{get_lang_suffix($item)}}" id="term_description_update{{get_lang_suffix($item)}}" class="form-control {{get_lang_class($key, $item)}}"
              placeholder="{{__('Description')}}" @if(!empty($item)) data-lang="{{$item}}" @endif>{{ get_translate($termObject->term_description, $item) }}</textarea>
    @endforeach
</div>
<div class="form-group field-upload">
    ...
    <div class="hh-upload-wrapper clearfix">
        ...
        <div class="mt-1">
            <a href="javascript:void(0);" class="remove-attachment">{{__('Supprimer')}}</a>
            <a href="javascript:void(0);" class="add-attachment"
               title="{{__('Add Image')}}"
               data-text="{{__('Add Image')}}"
               data-url="{{ dashboard_url('all-media') }}">{{__('Ajouter image')}}</a>
            <input id="term_image_update" type="hidden" class="upload_value input-upload" value="{{ $imageID }}" name="term_image"
