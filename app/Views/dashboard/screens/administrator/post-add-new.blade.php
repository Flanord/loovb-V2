...
@php
    ...
    $textPostHeading = __('Ajouter un nouveau article');
    $postStatus = 'publish';
                    ...
                    <div class="col-12 col-md-8 order-md-4">
                        <div class="card-box">
                            <h4 class="page-title">
                                {{ $textPostHeading }}
                                @if(!$addNewValue)
                                    <a target="_blank" href="{{ get_the_permalink($post_id, $current_post->post_slug, 'post') }}" class="right"><small>{{__("Voir l'article")}}</small></a>
                                @endif
                            </h4>
                            <hr />
                            ...
                            <div class="d-flex d-md-block d-xl-flex align-items-center mb-2 justify-content-between">
                                <div class="d-flex align-items-center form-xs">
                                    ...
                                    <select class="form-control min-w-100" id="hh-page-status" name="post_status" data-plugin="customselect">
                                        <option value="publish" {{ $postStatus == 'publish' ? 'selected' : ''  }}>{{__('Publier')}}</option>
                                        <option value="draft" {{ $postStatus == 'draft' ? 'selected' : ''  }}>{{__('brouillon')}}</option>
                                    </select>
                                </div>

                                <button class="btn btn-success waves-effect waves-light mb-0 mt-md-2 mt-xl-0" type="submit">{{__('Publier')}}</button>
                            </div>
                            @if(!empty($post_id) && $post_id != -1)
		                        <?php
		                        $data = [
                                ...
                                <a class="hh-link-action hh-link-delete-post d-inline-flex align-items-center a-red"
                                   ...
                                   data-confirm-question="{{__('Voulez-vous vraiment supprimer ce message?')}}"
                                   data-confirm-button="{{__('Supprime-le!')}}"
                                   href="javascript: void(0)">
                                   <i class="ti-trash mr-1"></i>
                               {{__('Supprimer cet artcile')}}</a>
                            @endif
                            <hr />
