...
@php
    ...
    $textPageHeading = __('Ajouter une nouvelle page');
    $pageStatus = 'publish';
                    ...
                    <div class="col-12 col-md-8 order-md-4">
                        <div class="card-box">
                            <h4 class="page-title">
                                {{ $textPageHeading }}
                                @if(!$addNewValue)
                                <a target="_blank" href="{{ get_the_permalink($post_id, $current_page->post_slug, 'page') }}" class="right"><small>{{__('Voir page')}}</small></a>
                                @endif
                            </h4>
                            <hr />
                            ...
                            <div class="d-flex d-md-block d-xl-flex align-items-center mb-2 justify-content-between">
                                <div class="d-flex align-items-center form-xs">
                                    ...
                                    <select class="form-control min-w-100" id="hh-page-status" name="post_status" data-plugin="customselect">
                                        <option value="publish" {{ $pageStatus == 'publish' ? 'selected' : ''  }}>{{__('Publier')}}</option>
                                        <option value="draft" {{ $pageStatus == 'draft' ? 'selected' : ''  }}>{{__('Brouillon')}}</option>
                                    </select>
                                </div>
