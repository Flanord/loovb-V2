...
class Car extends Model
{
    ...
    public function getAllCars($data)
    {
        ...
        if (!empty($data['status'])) {
            $sql->where('car.status', $data['status']);
        }
    ...
    public function deleteCarItem($car_id)
    {
        ...
        DB::table('term_relation')->where('service_id', $car_id)->delete();
    ...
    public function duplicate($car_id){
        ...
        if(!is_null($car)){
            ...
            if($new_car){
                return DB::insert(DB::raw("INSERT INTO term_relation ( term_id, service_id ) SELECT
                    t.term_id, {$new_car}
                    FROM
                        term_relation AS t
                    WHERE
